...
main_menu() {
    while true; do
        CHOICE=$(dialog --clear --backtitle "$BACKTITLE" --title "Menu Inicial do Kat#BASHes" \
            --menu "Escolha uma opção:" 18 60 9 \
            1 "Jogar" \
            2 "Jogos Salvos" \
            3 "Sobre" \
            4 "NIS(TM) Store" \
            ...
            7 "[⚠] Fim do Suporte ao Kat#BASHes" \
            8 "Sair" \
            9 "PWD" \
            2>&1 >/dev/tty)
...
Versão: Milestone (pre-Alpha) 5
Linha: # (Linha 'Tag')

Feito por: StanoXBMC
Distribuído por: Neko Interactive Systems(TM) + StanoXBMC
...
updateservice() {
    URL="https://github.com/NT-Stan1996/Kat-BASHes/raw/main/0-Milestone/6/KTBASHes.zip"
    if curl --output /dev/null --silent --head --fail "$URL"; then
        ...
        dialog --backtitle "Kat#BASHes" --msgbox "Nenhuma atualização disponível no momento.\nO Kat#BASHes foi descontinuado, vá para o menu e selecione a opção 7 para mais informações." 10 50
    fi
}
...
detect_arch() {
    ...
    case "$RAW_ARCH" in
        aarch64) echo "ARM64" ;;
        armv7l|armv8l|armhf|arm) echo "ARM" ;;
        x86_64) echo "64" ;;
        i386|i686) echo "32" ;;
        *) echo "DESCONHECIDA" ;;
    esac
...
eos_notice() {
ARCH_HUMAN=$(detect_arch)

dialog --backtitle "[⚠] Fim do Suporte ao Kat#BASHes" --msgbox "$(cat <<EOF
O suporte ao Kat#BASHes está oficialmente entrando em fase final.

→ O sistema receberá apenas correções mínimas até o início de 2026,
  sem aumento de versão. Depois disso, será movido para a categoria
  LEGACY. Ele NÃO será removido — só deixará de ser o ambiente
  principal de execução.

Nova geração:
• Kat#ent (com desempenho nativo e suporte avançado)
• Versões por arquitetura:
  - KatRTi //// ARM64
  - KatRTI //// ARM
  - KatRTi //// x86_64
  - KatRTi //// x86

Edição recomendada para seu dispositivo:
KatRTi //// ${ARCH_HUMAN}

→ Compatibilidade total:
Todos os apps e jogos do Kat#BASHes continuarão funcionando dentro
do Sandbox de Compatibilidade do KatRTi. Provavelmente nada será perdido.

Obrigado por acompanhar o Kat#BASHes até aqui.

— StanoXBMC (anteriormente StanoBemLoko)

© 2019-2025 TGF! Studios™
© 2024-2025 Neko Interactive Systems™
EOF
)" 25 70
}
